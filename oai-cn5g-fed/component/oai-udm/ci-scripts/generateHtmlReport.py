...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_udm.html', 'w')
		self.generateHeader()
	...
	def generateHeader(self):
		...
		for reportFile in glob.glob('./*results_oai_*.html'):
			if reportFile == './test_results_oai_udm.html':
				continue
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI UDM <font color="Gold">Ubuntu18</font> Image</th>\n')
		self.file.write('		<th>OAI UDM <font color="Gold">RHEL8</font> Image</th>\n')
		self.file.write('	  </tr>\n')
	...
	def initialGitSetup(self):
		...
		self.analyze_docker_build_git_part('UDM')
		self.file.write('	 </tr>\n')
	...
	def analyze_docker_build_git_part(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				section_end_pattern = 'WORKDIR /openair-udm/build/scripts'
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'build_udm --install-deps --force'
				section_end_pattern = 'build_udm --clean --Verbose --build-type Release --jobs'
				section_status = False
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search('FROM oai-udm-base', line)
						if result is not None:
							base_image = True
						result = re.search(section_start_pattern, line)
						if result is not None:
						...
						if result is not None:
							section_status = False
						if section_status:
							result = re.search('UDM deps installation successful', line)
							if result is not None:
								status = True
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_udm --install-deps --force\n'
				if base_image:
	...
	def buildCompileRows(self):
		...
		self.analyze_build_log('UDM', True)
		self.file.write('	 </tr>\n')
		self.file.write('	 <tr>\n')
		self.analyze_compile_log('UDM', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_build_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				if nfType == 'UDM':
					section_start_pattern = 'build_udm --clean --Verbose --build-type Release --jobs'
					section_end_pattern = '[aA][sS] oai-udm$'
					pass_pattern = 'udm installed'
				section_status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search(section_start_pattern, line)
					...
					for line in logfile:
						...
						if section_status:
							...
							if result is not None:
								nb_warnings += 1
								if nfType == 'UDM':
									correctLine = re.sub("^.*/openair-udm","/openair-udm",line.strip())
	...
	def copyConfToolsToTargetImage(self):
		...
		self.analyze_copy_conf_tool_log('UDM')
		self.file.write('	 </tr>\n')
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('UDM', True)
		self.file.write('	 </tr>\n')
