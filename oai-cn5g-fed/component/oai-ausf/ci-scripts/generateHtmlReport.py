...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_ausf.html', 'w')
		self.generateHeader()
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search('FROM oai-ausf-base', line)
						if result is not None:
							base_image = True
						result = re.search(section_start_pattern, line)
						if result is not None:
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_ausf --install-deps --force\n'
				if base_image:
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('AUSF', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_image_size_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				if nfType == 'AUSF':
					if variant == 'docker':
						section_start_pattern = 'Successfully tagged oai-ausf'
						section_end_pattern = 'OAI-AUSF DOCKER IMAGE BUILD'
					else:
						section_start_pattern = 'COMMIT oai-ausf:'
						section_end_pattern = 'OAI-AUSF PODMAN RHEL8 IMAGE BUILD'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if section_status:
							if nfType == 'AUSF':
								if self.git_pull_request:
									result = re.search('oai-ausf *ci-tmp', line)
								else:
									result = re.search('oai-ausf *develop', line)
							if result is not None:
								if variant == 'docker':
