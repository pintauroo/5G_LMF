...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_udr.html', 'w')
		self.generateHeader()
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI UDR <font color="Gold">Ubuntu18</font> Image</th>\n')
		self.file.write('		<th>OAI UDR <font color="Gold">RHEL8</font> Image</th>\n')
		self.file.write('	  </tr>\n')
	...
	def initialGitSetup(self):
		...
		self.analyze_docker_build_git_part('UDR')
		self.file.write('	 </tr>\n')
	...
	def analyze_docker_build_git_part(self, nfType):
		...
		for variant in variants:
			logFileName = 'udr_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'git config --global http'
				section_end_pattern = 'WORKDIR /openair-udr/build/scripts'
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'build_udr --install-deps --force'
				section_end_pattern = 'build_udr --clean --Verbose --build-type Release --jobs'
				section_status = False
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_udr --install-deps --force\n'
				if base_image:
	...
	def buildCompileRows(self):
		...
		self.analyze_build_log('UDR', True)
		self.file.write('	 </tr>\n')
		self.file.write('	 <tr>\n')
		self.analyze_compile_log('UDR', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_build_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				if nfType == 'UDR':
					section_start_pattern = 'build_udr --clean --Verbose --build-type Release --jobs'
					section_end_pattern = '[aA][sS] oai-udr$'
					pass_pattern = 'udr installed'
				section_status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search(section_start_pattern, line)
					...
					for line in logfile:
						...
						if section_status:
							...
							if result is not None:
								nb_warnings += 1
								if nfType == 'UDR':
									correctLine = re.sub("^.*/openair-udr","/openair-udr",line.strip())
	...
	def analyze_copy_log(self, nfType):
		...
		for variant in variants:
			logFileName = 'udr_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				section_start_pattern = '[aA][sS] oai-udr$'
				section_end_pattern = 'WORKDIR /openair-udr/etc'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
	...
	def copyConfToolsToTargetImage(self):
		...
		self.analyze_copy_conf_tool_log('UDR')
		self.file.write('	 </tr>\n')
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('UDR', True)
		self.file.write('	 </tr>\n')
