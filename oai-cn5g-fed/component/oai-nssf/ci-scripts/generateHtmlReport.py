...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_nssf.html', 'w')
		self.generateHeader()
	...
	def generateHeader(self):
		...
		for reportFile in glob.glob('./*results_oai_*.html'):
			if reportFile == './test_results_oai_nssf.html':
				continue
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI NSSF <font color="Gold">Ubuntu18</font> Image</th>\n')
		self.file.write('		<th>OAI NSSF <font color="Gold">RHEL8</font> Image</th>\n')
		self.file.write('	  </tr>\n')
	...
	def initialGitSetup(self):
		...
		self.analyze_docker_build_git_part('NSSF')
		self.file.write('	 </tr>\n')
	...
	def analyze_docker_build_git_part(self, nfType):
		...
		for variant in variants:
			logFileName = 'nssf_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'git config --global http'
				section_end_pattern = 'WORKDIR /openair-nssf/build/scripts'
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'build_nssf --install-deps --force'
				section_end_pattern = 'build_nssf --clean --Verbose --build-type Release --jobs'
				section_status = False
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if result is not None:
							section_status = False
						if section_status:
							result = re.search('NSSF deps installation successful', line)
							if result is not None:
								status = True
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_nssf --install-deps --force\n'
				if base_image:
	...
	def buildCompileRows(self):
		...
		self.analyze_build_log('NSSF', True)
		self.file.write('	 </tr>\n')
		self.file.write('	 <tr>\n')
		self.analyze_compile_log('NSSF', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_build_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				if nfType == 'NSSF':
					section_start_pattern = 'build_nssf --clean --Verbose --build-type Release --jobs'
					section_end_pattern = '[aA][sS] oai-nssf$'
					pass_pattern = 'nssf installed'
				section_status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search(section_start_pattern, line)
					...
					for line in logfile:
						...
						if section_status:
							...
							if result is not None:
								nb_warnings += 1
								if nfType == 'NSSF':
									correctLine = re.sub("^.*/openair-nssf","/openair-nssf",line.strip())
	...
	def analyze_copy_log(self, nfType):
		...
		for variant in variants:
			logFileName = 'nssf_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				section_start_pattern = '[aA][sS] oai-nssf$'
				section_end_pattern = 'WORKDIR /openair-nssf/etc'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
	...
	def copyConfToolsToTargetImage(self):
		...
		self.analyze_copy_conf_tool_log('NSSF')
		self.file.write('	 </tr>\n')
	...
	def analyze_copy_conf_tool_log(self, nfType):
		...
		for variant in variants:
			logFileName = 'nssf_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				section_start_pattern = 'WORKDIR /openair-nssf/etc'
				if variant == 'docker':
					section_end_pattern = 'Successfully tagged oai-nssf'
				else:
					section_end_pattern = 'COMMIT oai-nssf:'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('NSSF', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_image_size_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				if nfType == 'NSSF':
					if variant == 'docker':
						section_start_pattern = 'Successfully tagged oai-nssf'
						section_end_pattern = 'OAI-NSSF DOCKER IMAGE BUILD'
					else:
						section_start_pattern = 'COMMIT oai-nssf:'
						section_end_pattern = 'OAI-NSSF PODMAN RHEL8 IMAGE BUILD'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
